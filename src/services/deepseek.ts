...
export class DeepSeekService {
  ...
  async decideMultiStage(
    ...
    try {
      ...
      const analysisResponse = await this.queue.add(async () => {
        logger.debug('Ejecutando primera etapa (análisis) de DeepSeek');
      ...
      const decisionResponse = await this.queue.add(async () => {
        logger.debug('Ejecutando segunda etapa (decisión) de DeepSeek');
  ...
  private buildDecisionPrompt(
    ...
    if (recentFeedback.length > 0) {
      ...
      recentFeedback.forEach(entry => {
        feedbackSection += `\n- ${entry.result.action} a $${entry.result.entryPrice.toFixed(2)}: ${
          entry.successful ? 'ÉXITO' : 'FRACASO'} (${
          entry.result.profitPercentage?.toFixed(2)}%, ${
          entry.result.holdDuration ? `duración: ${entry.result.holdDuration} min` : 'sin datos de duración'})`;
      });
    }
...
- Ganancia promedio: ${symbolStats.avgProfit.toFixed(2)}%`;
    ...
    if (recentFeedback.length > 0) {
      prompt += `\n\nRESULTADOS RECIENTES:`;
      recentFeedback.forEach(entry => {
        prompt += `\n- ${entry.result.action} a $${entry.result.entryPrice.toFixed(2)}: ${entry.successful ? 'ÉXITO' : 'FRACASO'} (${entry.result.profitPercentage?.toFixed(2)}%)`;
      });
    }
