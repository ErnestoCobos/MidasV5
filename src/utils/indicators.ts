import { RSI, EMA, BollingerBands, SMA, ROC, ATR, MACD } from 'technicalindicators';
...
export class TechnicalIndicators {
   ...
   */
  static calculateROC(candles: Candle[], period = 9): number {
    try {
      const closes = candles.map(candle => candle.close);
      ...
      return rocValues.pop() ?? 0;
    } catch (error) {
      logger.error({ error }, 'Error calculando ROC');
      return 0;
    }
  }
  ...
  static calculateATR(candles: Candle[], period = 14): number {
    try {
      ...
      logger.error({ error }, 'Error calculando ATR');
  ...
  static calculateMomentumScore(candles: Candle[]): number {
    try {
      ...
      logger.error({ error }, 'Error calculando Momentum Score');
  ...
  static calculateKellyFraction(
    ...
    reward: number, 
    risk: number,
    fractionMultiplier: number = 0.3  // Fractional Kelly para reducir riesgo
  ): number {
    try {
      // Validar entradas
      ...
      logger.error({ error }, 'Error calculando Kelly Fraction');
   ...
   */
  static calculateGrowthPotential(candles: Candle[]): number {
    try {
      // Calcular componentes
      ...
      logger.error({ error }, 'Error calculando Growth Potential');
      return 50; // Valor neutral
    }
  }
  ...
  static calculateTakeProfitLevels(
    ...
    try {
      ...
      logger.error({ error }, 'Error calculando Take Profit Levels');
